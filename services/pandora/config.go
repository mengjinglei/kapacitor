...
type Config struct {
	...
	Ak      string   `toml:"ak" override:"ak"`
	Sk      string   `toml:"sk" override:"sk,redact"`
	// Path to CA file
...
func (c *Config) Init() {
	c.Name = "default"
	c.URLs = []string{"https://tsdb.qiniu.com:80"}
...
func (c Config) Validate() error {
	if c.Name == "" {
		return errors.New("Pandora TSDB must be given a name")
	}
	if !validNamePattern.MatchString(c.Name) {
		return errors.New("Pandora TSDB name must contain only numbers, letters or '.', '-', and '_' characters")
	}
	if len(c.URLs) == 0 {
		return errors.New("must specify at least one Pandora TSDB URL")
	}
